...
public abstract class RoomMembersAdapter extends ArrayAdapter<RoomMember> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        TextView textView = (TextView) convertView.findViewById(org.matrix.console.R.id.roomMembersAdapter_name);
        ...
        textView = (TextView) convertView.findViewById(0); //FIXME SACES org.matrix.console.R.id.roomMembersAdapter_membership);
        ...
        textView = (TextView) convertView.findViewById(org.matrix.console.R.id.roomMembersAdapter_userId);
        textView.setText(member.getUserId());

        ImageView imageView = (ImageView) convertView.findViewById(org.matrix.console.R.id.avatar_img);
        imageView.setTag(null);
        imageView.setImageResource(org.matrix.console.R.drawable.ic_contact_picture_holo_light);
        ...
        if (!TextUtils.isEmpty(url)) {
            int size = getContext().getResources().getDimensionPixelSize(org.matrix.console.R.dimen.member_list_avatar_size);
            mMediasCache.loadAvatarThumbnail(mHsConfig, imageView, url, size);
        }

        // The presence ring
        ImageView presenceRing = (ImageView) convertView.findViewById(org.matrix.console.R.id.imageView_presenceRing);
        ...
        PieFractionView pieFractionView = (PieFractionView) convertView.findViewById(0); // FIXME SACES R.id.powerDisc);
    ...
    public static String buildLastActiveDisplay(Context context, long lastActiveAgo) {
        ...
        if (lastActiveAgo < 60) {
            return context.getString(org.matrix.console.R.string.last_seen_secs, lastActiveAgo);
        }

        lastActiveAgo /= 60; // In minutes
        if (lastActiveAgo < 60) {
            return context.getString(org.matrix.console.R.string.last_seen_mins, lastActiveAgo);
        }

        lastActiveAgo /= 60; // In hours
        if (lastActiveAgo < 24) {
            return context.getString(org.matrix.console.R.string.last_seen_hours, lastActiveAgo);
        }

        lastActiveAgo /= 24; // In days
        return context.getString(org.matrix.console.R.string.last_seen_days, lastActiveAgo);
    }
}
